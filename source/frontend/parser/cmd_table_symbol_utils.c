...
/*   Updated: 2024/06/07 16:01:40 by ldulling         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_bracket(t_tok_typ token_type, t_list_d *cmd_table_list)
{
	...
	cmd_table = ft_lstlast_d(cmd_table_list)->content;
	if (token_type == T_L_BRACKET)
		cmd_table->type = C_SUBSHELL_START;
	else if (token_type == T_R_BRACKET)
...
static bool	fill_redirect(t_list *token_list, t_ct *cmd_table)
{
	...
	red_type = get_token_type_from_list(token_list);
	filename = token_list->next->content;
	io_red = init_io_red();
	if (!io_red)
		return (false);
	if (!fill_red_node(io_red, red_type, filename->data))
